...
## Patterns / UX
- Hero background: desktop использует видео `/hirovideo.mp4`; mobile использует `/hiro mobil1.png` как `object-cover` без blur/градиентов.
...
- Каталог показывает 6 карточек по умолчанию; кнопка переключает «Показать весь ассортимент» / «Свернуть ассортимент».
- В каталоге обновлены позиции: «Махорка "Крымская" Крым» (40г), «Махорка "Золотая" Россия» (40г), «Махорка "СССР" Россия» (50г); карточка «Махорка Perique Blend» удалена.
...
- Общий фон секций через `fon.webp`: класс `.fon-bg` (в `src/index.css`) задаёт `background-image: url('/fon.webp')` + overlay `::before`; применён к секции `#catalog`.
- Секция “Качество и происхождение сырья” использует фон `fon.webp` через класс `.fon-bg` (без видео).
- Для `#catalog` используется статичный фон `fon.webp` через класс `.fon-bg` (без анимаций).
