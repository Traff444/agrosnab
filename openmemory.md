...
## Patterns / UX
- Hero background: desktop использует видео `/1234.mp4`; mobile использует статичную картинку `/mobilHero.webp`.
- Карточки ассортимента (`ProductCard` в секции `#catalog`) используют локальные обложки из `public/assortiment1.webp`…`assortiment4.webp` (циклически для всего списка, чтобы стиль был единым и без внешних ссылок).
- `ProductCard`: изображение в карточке сейчас рендерится как `object-cover` с hover-увеличением (`group-hover:scale-105`).
